var angular;
var goldenApp = angular.module('goldenApp', ['ui.router', 'ngAnimate', 'ngResource', 'ngNotify', 'ngSanitize']);
...
.run(['$http', '$templateCache', 'ngNotify', function($http, $templateCache, ngNotify){
   ngNotify.config({
      default: 'info',
      duration: 4000,
      position: 'bottom',
   ...
   $http.get('/views/templates/print.ejs').then(function(response){
      $templateCache.put('print.ejs', response.data);
   });
}]);
